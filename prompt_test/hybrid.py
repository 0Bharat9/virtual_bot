...
class TextToSpeech:
    ...
    def speak(self, text):
        ...
        DEEPGRAM_URL = f"https://api.deepgram.com/v1/speak?model={self.MODEL_NAME}&performance=false&encoding=linear16&sample_rate=24000"
        headers = {
            ...
            with requests.post(DEEPGRAM_URL, stream=True, headers=headers, json=payload) as r:
                ...
                for chunk in r.iter_content(chunk_size=1024):
                    if chunk:
                        ...
                        #print(f"Received chunk of size: {len(chunk)}")  # Debug chunk size
                        player_process.stdin.write(chunk)
                        player_process.stdin.flush()

        except Exception as e:
            print(f"Error occurred during request or playback: {e}")
...
# New function for recording audio and saving it to a temporary file
def record_audio():
    suppress_alsa_warnings()
...
async def get_transcript(callback):
    wav_path = record_audio()  # Record audio
